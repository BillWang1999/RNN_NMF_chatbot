...
def trainIters(model_name, voc, pairs, pairs_validation, encoder, decoder, encoder_optimizer, decoder_optimizer, embedding, encoder_n_layers, decoder_n_layers, save_dir, n_iteration, batch_size, print_every, save_every, clip, corpus_name, loadFilename):

    #history record file
    history_file = open(r'C:\Users\wxwyl\Desktop\wylcode\cornell-movie\history.txt','w')

    # Load batches for each iteration
    training_batches = [p.batch2TrainData(voc, [random.choice(pairs) for _ in range(batch_size)])
                      for _ in range(n_iteration)]
    training_batches_validation = [p.batch2TrainData(voc, [random.choice(pairs_validation) for _ in range(batch_size)])
                        for _ in range(n_iteration)]
...
def my_random_pick(probabilities):
    ...
    for item in range(len(probabilities)):
        cumulative_probability += probabilities[item][0]
        if x < cumulative_probability:
            break
    item_list = torch.zeros([1], device=device, dtype=torch.long)
    item_list[0] = item
    return item_list, probabilities[item]
